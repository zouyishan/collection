...
entry start
start:
	! 设置附加数据段ES指向SETUPSEG，以便正常显示字符串数据
	mov ax, #SETUPSEG
	mov es, ax
	! 页号0，获取光标位置
	mov ah, #0x03
	...
	mov cx, #34 ! 字符串的长度
	mov bx, #0x0007 ! 页号bh是0，属性bl是7(normal，正常显示)
	mov bp, #msg1	! 显示ES:BP要显示的数据的地址
	mov ax, #0x1301 ! AH13表示要显示字符串，AL01表示光标跟随移动
	int 0x10	! 10中断打印
...
msg1: 
	.byte 13,10
	.ascii "Now we are in VirtualR SETUP"
	.byte 13,10,13,10

Cursor:
	.ascii "VirtualR System Cursor POS:"	!0x90000 2bytes
Memory:
	.ascii "VirtualR System Memory SIZE:"	!0x90002 2bytes
Cyl_hd0:
